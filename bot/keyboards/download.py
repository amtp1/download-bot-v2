...
IKB_SELECT_TYPE = InlineKeyboardMarkup(
    inline_keyboard=[
        [InlineKeyboardButton(text="Audio", callback_data=BasicCallback(action=BasicAction.SELECT_AUDIO).pack())],
        [InlineKeyboardButton(text="Video", callback_data=BasicCallback(action=BasicAction.SELECT_VIDEO).pack())],
    ]
)
...
def convert_to_content_types(title: str, streams: dict, is_audio=None, is_video=None):
    ...
    for i, item in enumerate(items):
        ...
        inline_button = InlineKeyboardButton(text=f"{mime_type} - {item['sizeText']}",
